...
def read_ow_and_publish(client: MQTTClient):
    ...
    for i, record in enumerate(temps):
        sensor_name = config.sensor_name_map[config.dhcp_hostname][i]
        topic = f"{config.mqtt_group}/{config.mqtt_device}/sensor/{sensor_name}"
...
mqtt = mqtt_connect()
tim = machine.Timer(1)
tim.init(period=config.sample_period_ms, callback=lambda t: read_ow_and_publish(mqtt))
